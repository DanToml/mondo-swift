...
/**
 *  `Client` exposes methods to allow you to easily use the Monzo Banking API.
 *
...
public class Client {
    ...
    /**
     ...
     - parameter baseURL:     The baseURL to use for requests, optional, defailts to https://api.monzo.com
     */
    public init(accessToken: String, baseURL: NSURL = NSURL(string: "https://api.monzo.com")!) {
...
extension Client {
    public func requestTransaction(withIdentifier identifier: String, completion: Result<NSDictionary> -> ()) -> NSURLSessionTask {
        let endpoint = MonzoEndpoints.Transactions.findWithIdentifier(accessToken, transactionIdentifier: identifier)
    ...
    public func requestTransactions(forAccount accountIdentifier: String, completion: Result<NSDictionary> -> ()) -> NSURLSessionTask  {
        let endpoint = MonzoEndpoints.Transactions.listForAccount(accessToken, accountIdentifier: accountIdentifier)
        return request(endpoint, completion: completion)
    }
}
...
extension Client {
    public func requestBalance(forAccount accountIdentifier: String, completion: Result<NSDictionary> -> ()) -> NSURLSessionTask {
        let endpoint = MonzoEndpoints.Balance.forAccount(accessToken, accountIdentifier: accountIdentifier)
        return request(endpoint, completion: completion)
    }
}
